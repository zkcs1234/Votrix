...
export default function CreateOrganizerPage() {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (data.email?.sent) {
        ...
        setEmailStatus('Account created. Email not sent â€” configure RESEND_API_KEY.')
      } else {
  ...
  return (
    <div className="mx-auto max-w-lg">
      <Link to="/admin" className="text-sm text-v-text-muted hover:text-v-text">
        â† Back to admin
      </Link>

      <h2 className="mt-4 text-lg font-semibold text-v-text">Create organizer</h2>
