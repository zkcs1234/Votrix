...
describe('scoring engine: computeRankings', () => {
  ...
  test('tie with no tie-breaker: contestants share a rank (1224)', () => {
    const scores = [
      { contestant_id: 'c1', criteria_id: 'k1', score: 70, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k2', score: 70, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 70, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k2', score: 70, judge_id: 'j1' },
    ]
    const { rankings } = computeRankings({ scores, contestants, criteria })
    expect(rankings.every((r) => r.finalScore === 70)).toBe(true)
    expect(rankings[0].rank).toBe(1)
...
describe('scoring engine: Option B — methods', () => {
  const contestants = [
    { id: 'c1', name: 'Alice', contestant_number: 1 },
    { id: 'c2', name: 'Bob', contestant_number: 2 },
  ]

  test('trimmed_average matches lowest_removal math', () => {
    expect(
      reduceScores([10, 20, 30, 40, 50], {
        calculationMethod: CALCULATION_METHODS.TRIMMED_AVERAGE,
        dropLowest: 1,
        dropHighest: 1,
      }),
    ).toBe(30)
  ...
  test('judge weighting: weighted mean across judges when enabled', () => {
    const criteria = [{ id: 'k1', name: 'Overall', percentage: 100 }]
    const scores = [
      { contestant_id: 'c1', criteria_id: 'k1', score: 100, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 0, judge_id: 'j2' },
    ]
    const { rankings } = computeRankings({
      scores,
      contestants: [contestants[0]],
      criteria,
      config: { calculationMethod: CALCULATION_METHODS.AVERAGE, judgeWeightingEnabled: true },
      judgeWeights: { j1: 75, j2: 25 },
    })
    // (100*75 + 0*25) / 100 = 75  (equal average would be 50)
    expect(rankings[0].finalScore).toBe(75)
  })

  test('judge weighting off: equal average regardless of weights passed', () => {
    const criteria = [{ id: 'k1', name: 'Overall', percentage: 100 }]
    const scores = [
      { contestant_id: 'c1', criteria_id: 'k1', score: 100, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 0, judge_id: 'j2' },
    ...
    const { rankings } = computeRankings({
      ...
      contestants: [contestants[0]],
      criteria,
      config: { calculationMethod: CALCULATION_METHODS.AVERAGE },
      judgeWeights: { j1: 75, j2: 25 },
    })
    expect(rankings[0].finalScore).toBe(50)
  })

  test('percentile: order wins even when a judge scores on a different scale', () => {
    const criteria = [{ id: 'k1', name: 'Overall', percentage: 100 }]
    const scores = [
      // j1 scores high, j2 scores low, but BOTH rank c1 over c2.
      { contestant_id: 'c1', criteria_id: 'k1', score: 100, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 90, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 20, judge_id: 'j2' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 10, judge_id: 'j2' },
    ]
    const { rankings } = computeRankings({
      scores,
      contestants,
    ...
    expect(rankings[0].contestantId).toBe('c1')
    expect(rankings[0].finalScore).toBe(75) // top percentile within each judge
    expect(rankings[1].finalScore).toBe(25)
  })

  test('rank_based: only the order a judge gives matters, not the magnitude', () => {
    const criteria = [{ id: 'k1', name: 'Overall', percentage: 100 }]
    const scores = [
      // Both judges rank c1 first; raw magnitudes differ a lot.
      { contestant_id: 'c1', criteria_id: 'k1', score: 60, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 59, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 100, judge_id: 'j2' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 10, judge_id: 'j2' },
    ]
    const { rankings } = computeRankings({
      scores,
    ...
    expect(rankings[0].contestantId).toBe('c1')
    expect(rankings[0].finalScore).toBe(100) // always-first → full points
    expect(rankings[1].finalScore).toBe(0)
  })
})

describe('scoring engine: Option B — tie-breakers', () => {
  const contestants = [
    { id: 'c1', name: 'Alice', contestant_number: 1 },
    { id: 'c2', name: 'Bob', contestant_number: 2 },
  ]

  test('highest_criterion breaks a tie by best single criterion', () => {
    const criteria = [
      { id: 'k1', name: 'A', percentage: 50 },
      { id: 'k2', name: 'B', percentage: 50 },
    ]
    const scores = [
      { contestant_id: 'c1', criteria_id: 'k1', score: 100, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k2', score: 40, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 70, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k2', score: 70, judge_id: 'j1' },
    ]
    const { rankings } = computeRankings({
    ...
    // both average 70, but c1's best single criterion (100) > c2's (70)
    expect(rankings[0].contestantId).toBe('c1')
    expect(rankings[0].rank).toBe(1)
    expect(rankings[1].rank).toBe(2)
  })

  test('judges_majority breaks a tie by how many judges scored higher', () => {
    const criteria = [{ id: 'k1', name: 'Overall', percentage: 100 }]
    const scores = [
      // c1 and c2 both average 70, but 2 of 3 judges scored c1 higher.
      { contestant_id: 'c1', criteria_id: 'k1', score: 90, judge_id: 'j1' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 90, judge_id: 'j2' },
      { contestant_id: 'c1', criteria_id: 'k1', score: 30, judge_id: 'j3' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 80, judge_id: 'j1' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 80, judge_id: 'j2' },
      { contestant_id: 'c2', criteria_id: 'k1', score: 50, judge_id: 'j3' },
    ]
    const { rankings } = computeRankings({
      scores,
      contestants,
      criteria,
      config: { tieBreaker: 'judges_majority' },
    })
    expect(rankings[0].finalScore).toBe(70)
    expect(rankings[1].finalScore).toBe(70)
    expect(rankings[0].contestantId).toBe('c1')
    expect(rankings[0].rank).toBe(1)
    expect(rankings[1].rank).toBe(2)
  })
